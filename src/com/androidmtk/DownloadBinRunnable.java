...
public class DownloadBinRunnable implements Runnable {
	...
	public DownloadBinRunnable(String file_time_stamp, Handler downLoadDialogHandler) {
		...
		create_log_file = AndroidMTK.getSharedPreferences().getBoolean("createDebugPref", false);
	...
	public void run() {
    	...
    	gpsdev = new GPSrxtx(AndroidMTK.getmBluetoothAdapter(), GPS_bluetooth_id);
    	if (gpsdev.connect()) {
        	...
        	try {
				reply = gpsdev.waitForReply("PMTK182,3,6,", 10.0);
			} catch (IOException e2) {
        	...
        	if (log_full_method == 1) {
        		...
        		if (bytes_to_read > 0) {
                    ...
                    // Wait for reply from the device
                    try {
						reply = gpsdev.waitForReply("PMTK705,", 10.0);
					} catch (IOException e) {
						// Error reading... oops.. close connection and fail
						errorWhileDownloading();
						return;
        		...
        		// Wait for reply from the device
        		try {
					reply = gpsdev.waitForReply("PMTK182,3,8,", 10.0);
				} catch (IOException e) {
					// Error reading... oops.. close connection and fail
					errorWhileDownloading();
					return;
			...
			try {
				bin_output_stream = new BufferedOutputStream(new FileOutputStream(bin_file), SIZEOF_SECTOR);
            ...
            while (running && offset < bytes_to_read) {
                ...
                for (int j=0; j<number_of_message; j++) {
            		try {
						reply = gpsdev.waitForReply("PMTK182,8", 10.0);
					} catch (IOException e1) {
						// Error reading... oops.. close connection and fail
						errorWhileDownloading();
						return;
	...
	private void sendCreateGPX(String message) {
		Message msg = dHandler.obtainMessage();
		Bundle b = new Bundle();
		b.putString(AndroidMTK.CREATEGPX, message);
		msg.setData(b);
		dHandler.sendMessage(msg);
	}
